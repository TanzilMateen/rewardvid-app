...
export function Signup() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    password: '',
    referred_by: referralFromUrl 
  });

  // URL change hone par input update karein
  useEffect(() => {
    if (referralFromUrl) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Cleanup: Khali string ko null karein
    ...
    try {
      ...
      if(response.ok) {
        ...
        alert(data.message || "Registration failed. This email might already exist.");
      }
    } catch (error) {
      alert("Network Error: Could not connect to the live server. Please check your connection.");
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <form className="space-y-5" onSubmit={handleSignup}>
              ...
              <div className="space-y-2">
                ...
                {formData.referred_by && (
                  <p className="text-[10px] font-black text-blue-600 uppercase ml-1">✨ Referral Code Applied</p>
                )}
              </div>
              ...
              >
                {isLoading ? <Loader2 className="animate-spin mr-2" /> : "Create My Account"}
              </Button>
            </form>
