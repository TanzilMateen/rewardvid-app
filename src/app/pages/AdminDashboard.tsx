...
import { 
  ...
  ArrowLeftRight // Maine yahan fix kar diya hai
} from 'lucide-react';
...
export function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      const resW = await fetch("http://localhost:5000/admin/withdrawals", {
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (resW.ok) setRequests(await resW.json());

      const resU = await fetch("http://localhost:5000/admin/users", {
        headers: { "Authorization": `Bearer ${token}` }
      });
  ...
  const handleWithdrawal = async (id: number, status: 'Completed' | 'Rejected') => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/admin/update-withdrawal`, {
        method: "POST",
  ...
  const updateBalance = async (id: number, currentBalance: number) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/admin/update-user-balance`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${localStorage.getItem("token")}` },
  ...
  const toggleUserStatus = async (id: number, currentStatus: number) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/admin/toggle-user-status`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${localStorage.getItem("token")}` },
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto bg-[#f8fafc] min-h-screen">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        ...
        <div className="flex gap-3 bg-white p-2 rounded-2xl shadow-sm border">
          <Button 
            ...
            className={`rounded-xl font-black ${activeTab === 'withdrawals' ? 'bg-blue-600' : ''}`}
          >
          ...
          <Button 
            ...
            className={`rounded-xl font-black ${activeTab === 'users' ? 'bg-blue-600' : ''}`}
          >
                    ...
                    <tr key={req.id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className="p-6 flex gap-2 justify-center">
                        {req.status === 'Pending' ? (
                          ...
                          <Badge className={req.status === 'Completed' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}>{req.status}</Badge>
                        )}
                      </td>
                    </tr>
                  ))
                    ...
                    <tr key={user.id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className="p-6">
                        <div className="flex items-center gap-2">
                          ...
                          <Button onClick={() => updateBalance(user.id, user.balance)} variant="ghost" size="sm" className="h-7 w-7 p-0 rounded-full text-blue-600">
                            <Edit2 size={12} />
                          </Button>
                        </div>
                      </td>
