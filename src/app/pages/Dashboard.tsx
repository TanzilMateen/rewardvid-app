...
export function Dashboard() {
  ...
  const fetchUserStats = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (res.ok) {
        setStats({
          name: data.name || 'User',
          role: data.role || 'user', // Backend se role le rahe hain
  ...
  const handleRedeem = async (type: 'video' | 'referral') => {
    ...
    try {
      ...
      if (res.ok) { fetchUserStats(); alert("✨ Points converted to Cash!"); }
    } catch (e) { alert("Redeem failed."); }
  };

  const handleWithdraw = async () => {
    const amt = Number(withdrawData.amount);
    if (amt < 500) return alert("❌ Min. Rs. 500");
    ...
    try {
      ...
      if (res.ok) { alert("✅ Request Sent!"); fetchUserStats(); }
    } catch (err) { alert("Error."); } finally { setWithdrawLoading(false); }
  };
