  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.token) {
        ...
        localStorage.setItem("userName", data.user.name); // Storing name for Home display
        localStorage.setItem("userRole", data.user.role); // Storing role for Admin Panel access
        
        alert("Login Successful! Welcome back.");
        navigate("/"); // 🚀 Redirecting to Home as requested
      } else {
        alert(data.message);
      }
    } catch (error) {
      alert("Error: Please ensure the backend server is running (node server.js)");
    }
  };
